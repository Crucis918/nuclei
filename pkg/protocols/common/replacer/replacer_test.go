...
func TestReplacerReplace(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			values:   map[string]any{"test": "random"},
			expected: "{{base64_encode('random')}}",
		},
		{
			name:     "Basic",
			template: "{{test}} §hello§ {{data}}",
			values:   map[string]any{"test": "random", "hello": "world"},
		...
		},
		{
			...
			values:   map[string]any{"var1": "variable 1", "var2": "variable 2"},
			expected: "{{variable 1}} and §variable 2§",
		},
		{
			name:     "Space in variable name",
			template: "{{var 1}} has a space",
			values:   map[string]any{"var 1": "variable 1"},
		...
		},
		{
			...
			values:   map[string]any{"var1": "first variable", "var2": "second variable"},
